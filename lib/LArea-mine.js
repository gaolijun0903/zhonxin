window.LArea = function() {
    ...
    return e.prototype = {
        init: function(e) {
            switch (this.params = e, this.trigger = document.querySelector(e.trigger), this.trigger1 = document.querySelector(e.trigger1), e.valueTo && (this.valueTo = document.querySelector(e.valueTo)), this.keys = e.keys, this.type = e.type || 1, this.type) {
            case 1:
            case 2:
                break;
            default:
        ...
        getData: function(e) {
            var t = this;
            if ("object" == typeof t.params.data) t.data = t.params.data,
            e();
            else {
        ...
        bindEvent: function() {
            function e(e) {
                ...
                var o = d.gearArea.querySelector(".area_province").parentElement, 
                c = d.gearArea.querySelector(".area_city").parentElement, 
                v = d.gearArea.querySelector(".area_county").parentElement;
            ...
            function t() {
                switch (d.gearArea.querySelector(".area_province").setAttribute("val", d.value[0]), d.gearArea.querySelector(".area_city").setAttribute("val", d.value[1]), d.gearArea.querySelector(".area_county").setAttribute("val", d.value[2]), d.type) {
                case 1:
...
//              console.log(e.target)
                ...
                t.style.webkitTransitionDuration = t.style.transitionDuration = "0ms"
            }
            function r(e) {
                e.preventDefault();
                for (var t = e.target;;) {
            ...
            function i(e) {
                ...
                Math.abs(a) <= .2 ? t["spd_" + t.id] = 0 > a ? -.08 : .08 : Math.abs(a) <= .5 ? t["spd_" + t.id] = 0 > a ? -.16 : .16 : t["spd_" + t.id] = a / 2,
                t["pos_" + t.id] || (t["pos_" + t.id] = 0),
                n(t)
            }
            function n(e) {
                function t() {
                    e.style.webkitTransitionDuration = e.style.transitionDuration = "200ms",
                    r = !0
                }
                var a = 0,
                r = !1;
            ...
            d.getData(function() {
                ...
                d.trigger1.addEventListener("click", e)
            })
        },
        setGearTooth: function(e) {
            ...
            if (n > s && (n = s), i[t.index].setAttribute("data-len", r), r > 0) {
                ...
                case 2:
                    ...
                    for (var v in c) if (v == o) {
                        d = c[v];
                        break
                    }
                }
                for (var l = "",
                v = 0; r > v; v++) l += "<div class='tooth'  ref='" + a[v][this.keys.id] + "'>" + a[v][this.keys.name] + "</div>";
                if (i[t.index].innerHTML = l, i[t.index].style["-webkit-transform"] = "translate3d(0," + 2 * -n + "em,0)", i[t.index].setAttribute("top", 2 * -n + "em"), i[t.index].setAttribute("val", n), t.index++, t.index > 2) return void(t.index = 0);
                t.setGearTooth(d)
            } else i[t.index].innerHTML = "<div class='tooth'></div>",
            i[t.index].setAttribute("val", 0),
            1 == t.index && (i[2].innerHTML = "<div class='tooth'></div>", i[2].setAttribute("val", 0)),
            t.index = 0
        },
        finish: function(e) {
            var t = this,
            ...
            t.value = [n, o, l],
            this.valueTo && (this.valueTo.value = d + (v ? "," + v: "") + (h ? "," + h: "")),
            t.close(e)
        },
        close: function(e) {
            e.preventDefault();
            ...
            t.trigger.dispatchEvent(a),
            document.body.removeChild(t.gearArea)
        }
    },
    e
} ();